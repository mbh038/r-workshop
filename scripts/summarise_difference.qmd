...
-   `<fct>` means 'factor', which is R-speak for categorical variable.
...
In this chunk we are going to use the 'pipe' operator `|>` (a vertial line , *not* a capital I, followed by a greater than symbol) . This is really useful and it certainly pays to get the hang of its use. Roughly speaking, it feeds the result of one line in to the next line. You can think of it as meaning, more or less, 'and then'.
...
-   calculate summary statistics, in this case the number of indivual penguins of each species, then the mean petal length and the standard error of the mean. We choose to do this because these values will help us address our question as to whether the petals lengths are different for different species.
...
The next stage in our 'workflow' towards doing a statistical analysis that addresses our question (Does the petal length of iris plants differ between species?) is to plot the data. There are many different plots we can do. Here we will show you how to do a box plot, a bar chart and a histogram, all of which can be useful for 'difference' questions, where the data fall into two or more categories and the response variable is numerical (count or continuous). Then we will illustrate the use of a scatter plot, which is useful when you are plotting continuous variables against each other (eg mass vs height), as we often want to do when investigating the data that arise from 'trend' questions.
...
-   the `facet_wrap()` line is really useful for creating groups of identically styled plots across all the levels of some categorical variable. Here, we used it create a histogram for each of the three species of iris and asked for them to be displayed in one column for easy comparison.

### Scatter plot

Box plots and bar charts are appropriate when one of the variables is categorical. In our case that variable was `Species`, which had threee levels: `setosa`, `versicolor` and `virginica`. If both variables we want to plot are continuous, and we want to investigate a trend rather than look for a difference, then we often want a scatter plot instead, and sometimes to add a 'best-fit' line through the data. This is how we would do that if we were, say, investigating the association between petal width and sepal width, separately for the three iris species:
...
```{r}
iris_data |>
  ggplot(aes(x=Petal.Width,y=Sepal.Width)) + #<1>
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  labs(x="Petal Width", y="Petal Length") +
  facet_wrap(~Species,ncol=3,scales = "free_x") + #<2>
  theme_classic()
```

## Pimping the plots

What you get served up from the code chunks above are pretty good plot, but they do have defficencies, as the comments beneath each of them may have suggested. Rest assued,yu cam cusomise these plots in a huge variety of ways to meet any reasonable need. Here are a few examples of ways to do that.

### Pimp One: Remove the legend

We sometimes get given a legend even though we don't need one, as in the bar chart above. To get rid of it, just add the line `theme(legend.position = "none")` to the bottom. Let's do it for that bar chart plot:
...
R recognises a large number of colour names (see [here](https://www.nceas.ucsb.edu/sites/default/files/2020-04/colorPaletteCheatsheet.pdf), for example), such as we used for the box plot above, and you may be happy with that choice, but you may wish to use other palettes of colour. One set of these that is widely used in charts is derived from work by the geographer Cynthia Brewer. See [here](https://r-graph-gallery.com/38-rcolorbrewers-palettes.html) and [here](https://colorbrewer2.org/#type=sequential&scheme=BuGn&n=3) for an overview. You can use these colours easily in plots made using the `gglpot2` package by adding a `scale_fill_brewer()` or `scale_colour_brewer()` line to the plot code, depending on whether you want to fill box plots and such like with colour, or to colour points in scatter plots.

Here is how we would do this in the bar chart:
...
1.  Use the Brewer palette called 'Set 2'. This is an example of a qualitative palette, where the colours stand out from each other but do not imply any ordering. See [here](https://r-graph-gallery.com/38-rcolorbrewers-palettes.html) for more information.
...
1.  Add jittered points on top. Try altering the width argument to any value between 0 and 1. The alpha argument alters the opacity of the points, where 0 means fully transparent and 1 means fully opaque. This can be useful to use where points might lie on top of each other and potentially give the impression that the sample size and distribution is not what it really is.
