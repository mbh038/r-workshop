---
title: 'General Linear Models: simple linear regression'
author: "Michael Hunt"
date: "21/03/2024"
output:
  html_document:
...
## Introduction 
A class of analytical models that you will use often go under the name **General Linear Models**. They include simple linear regression, multiple linear regression, ANOVA, ANCOVA, Pearson correlation and t-tests, among others.
...
Your task is to assess whether there is a significant linear trend in pH with time and if so to determiine the change in pH per decade over the forty year period from 1980 to 2020.
...
### Oregon stream temperatures vs elevation

```{r}
streams <- read_csv(here("data","oregon_streams_regression.csv"), skip = 5)
glimpse(streams)
```
...
streams |>
  ...
  scale_y_continuous(limits=c(5,21),
                     expand = expansion(mult = c(0, 0.1))) +
  geom_segment(aes(x = 0, xend = 4000, y = 16.3954-0.0017155*4000, yend = 16.3954-0.0017155*4000), linetype="dashed", colour = "gray80") +
  geom_segment(aes(x = 4000, xend = 4000, y = 5, yend = 16.3954-0.0017155*4000), linetype="dashed", colour = "gray80") +
  annotate(geom="text",x = 4000, y = 9.53, label = "9.53", vjust= -0.8, hjust = 0) +
  theme_cowplot()
```
```{r, warning = FALSE, message = FALSE}
streams |>
  ...
  scale_y_continuous(limits=c(5,21),
                     expand = expansion(mult = c(0, 0.1))) +
  geom_segment(aes(x = elevation, y =  16.3954 - 0.0017155 * elevation, xend=elevation, yend = temperature), linetype= "dashed", colour = "gray60") +
  theme_cowplot()
```
...
```{r}
streams.model <- lm(temperature ~ elevation, data = streams)
```

```{r}
autoplot(streams.model) + theme_cowplot()
```
...
```{r}
filepath <- here("data", "arctic_ice_extent_september.csv")
# filepath <- here("data", "N_09_extent_v3.0.csv")
# filepath <- here("data", "arctic_sea_ice_minimum.csv")
arctic_ice <- read_csv(filepath, skip = 4)
glimpse(arctic_ice)
```

```{r}
arctic_ice %>%
  ggplot(aes(x = year, y = extent_msqkm)) +
  geom_point() +
...
```{r}
sea_ice_model <- lm(extent_msqkm ~ year, data = arctic_ice)
```
...
```{r}
trees <- read_csv(here("data","tree_vol.csv"))
glimpse(trees)
```
...
```
```{r}
cor.test(trees$height,trees$diameter)
```

```{r}
trees.model1 <-lm(volume ~ diameter, data = trees)
```

```{r}
summary(trees.model1)
```
```{r}
trees.model2 <- lm(volume ~ diameter + height, data= trees)
```

```{r}
summary(trees.model2)
```
```{r}
anova(trees.model2,trees.model1)
```
