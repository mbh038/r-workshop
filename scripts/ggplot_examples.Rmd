...
In this exercise we are going to use the package `ggplot` which is part of the larger `tidyverse` package and can be used to produce a variety of commonly used plots. 
...
You need to read the examples in this  worksheet and fill in the missing code, or alter what is provided already, in the empty code chunks of the accompanying template script. Instructions for gettin that are given below.

Remember that this is a markdown document, so you can add extra text between the code chunks to explain what is going on. You can format format it too, if you wish, according to the very basic markdown rules for doing this. See Help/Markdown Quick Reference.`

this formatting is only useful if you 'knit' the script, by pressing on the knit button at the top of the script pane. Try this! I suggest you knit to html. This is how the worksheet you are working from was produced.
...
Note the use of the pipe operator `%>%`, here and throughout. Think of it as meaning `and then`. It feeds the outtput of one line into the function of the next line, where it is used as that function's first argument.
...
Now we add labels, titles and so on, using the line `labs(...)`. Note how we can actually write the arguments of this over several lines on the page, for clarity, and that the title
...
It can be useful to include some combination of titles, subtitles and captions if the figure is to be used as part of a presentation or poster, but if it to go in a report, you would normally only include a caption, and let the word-processing software do it.
...
penguins %>%
  ...
  scale_fill_brewer(palette="Set2") + # try other paleete, eg "Set1".
  theme_bw() +
...
Arguably, this is a better plot than the previous one because it excludes the potentially confusing reduncacy of using differnt colours for the diffrent species, when we already know which species is the subject of each plot.
...
We can also adapt this and what was done for the histograms and do a set of three, one for each species, using `facet_wrap()`:
...
Which is more useful in this case: the overlapping plots on one chart, or the separate charts done using `facet_wrap()`? Whaever you think here, the answer in other cases will sometimes be one, sometimes the other. Now you have the tools to enable you to try both and make the best choice.
