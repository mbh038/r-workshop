...
Our null hypothesis $H_0$ is that the two samples are drawn from populations with the same mean mass, and the alternate hypothesis $H_1$ is that the populations from which they are drawn do not have the same mwan mass:
...
The number of degrees of freedom in this case is $n_\text{A}+n_\text{B}-2=50+50-2=98$. A *t*-distribution with 98 degrees of freedom is indistinguishable from a normal distribution and you may recall from above that the threshold $t$ value  in a two-sided test for a *p*-value of 0.05 or less is 1.96. Our value here is less than that and hence we conclude that there is no evidence from the data at the 5% signficance level that males and females have different body masses.
...
Note that we are given the *t*-statistic, the number of degrees of freedom and the *p*-value, on the basis of which we decide whether or not to reject the null hypothesis which is that the samples are drawn from populations with the same mean value. Note too that we are given a 95% confidence interval. This is the range of values that would capture the true value of the difference between the means 95% of the time, if we repeated the trial many times over. If this range encompasses zero, then we have no grounds, at the 95% confidence level, to think that the difference between the means is anything other than zero. Whenever this confidence interval *does* encompass zero, as it does here, the *p*-value *will* also be greater than 0.05, as it is here.
...
```
This gives a *t*-statistic of 2.17 and *p*-value of 0.03, on the basis of which we would erroneously reject the null hypothesis that males and female squirrels on aveage have the same body mass. We would be wrong to think that this output gave us grounds to do that, since the data do not meet the criteria required for a *t*-test, and hence we should not believe the output if we use one. 
