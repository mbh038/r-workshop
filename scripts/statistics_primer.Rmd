...
This primer is indebted to the very helpful revision chapter in [Modern Statistics for the Life Sciences, Alen Grafen and Rosie Hails, OUP](https://global.oup.com/ukhe/product/modern-statistics-for-the-life-sciences-9780199252312?cc=gb&lang=en&).
...
$$
\bar{y}=\frac{y_1+y_2+\dots + y_n}{n}=\dfrac{\Sigma_{i=1}^n y_i}{n}
$$

In this equation, we suppose that we have a sample of $n$ women, with each individual woman denoted by $y_1, y_2\dots y_n$ or in general for any individual by $y_i$ where the subscript $i$ runs from $1$ to $n$. To find the mean value we first sum the $y_i$ from $i=1$ to $i=n$ and then divide the result by $n$. A compact way to denote the sum is to use the summation sign $\Sigma$ (sigma), as we do in the right-hand term of the equation
...
Note that, from the formula for the standard error $\frac{\sigma}{\sqrt{n}}$, we can see that:

* The larger the variability within a population, and hence the larger the value of $\sigma$, the larger will be the standard error
* the larger the sample sie $n$, the *smaller* will be the standard error.

So, standard errors, that is, the uncertainty with which a sample mean estimates a true population mean, will be:

* small for large samples and for samples drawn from populations with low instrinsic variability (ie those with low standard deviation)
* large for small samples and for samples drawn from populations with high instrinsic variability (ie those with large standard deviation)
...
#| fig.cap="Histogram of the weights of the entire population of adult grey seals in South West England. In reality, we would never be able to measure this, but if we could, it would very likely closely approximate a normal distribution, as shown here."
population_dist<-
  seal.population |>
  ggplot(aes(x=mass)) +
...
sampling_dist<-
  ...
  annotate("text",
           ...
           hjust=0,
           vjust=0,
           label="The true mean is at the\nmiddle of this.\nThe mean of our one sample\nis somewhere in there ",
           size=2.5,
           colour="darkred") +
  theme_cowplot() +
  theme_no_y() +
