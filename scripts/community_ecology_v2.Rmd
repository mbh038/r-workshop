...
#### Ordination plot of disimilarites

Another commonly used technique for spotting patterns of (dis)similarity between sites based on their flora or on environmental variables is to use some form of 'ordination'. Roughly speaking, these are plots that show in two dimensions patterns that exist among multidimensional data. In the dune case, the multiple dimensions are the many species. What patterns exist among them? Which seem to like similar conditions and tend to appear at the same sites, and vice versa. Also, which sites are similar to each other, in respect of what grows there, and which are different? These are the questions that ordination plots are intended to shed light on. For data based on the presence/absence/abundance of species, Non-Metric Multidimensional Scaling (NMDS) is a widely used type of ordination technique.


Here, we will entirely skate over the details, but will carry out the analysis using the `vegan` function `metaMDS()` hen plot the result.
