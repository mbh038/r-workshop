...
### Detail of what is meant by the F value and R^2^
...
```{r}
grand_mean = mean(pH$pH)
SST = sum((pH$pH-grand_mean)^2)
SST
```
...
## Simple linear regression: Soil carbon from soil organic matter

A common goal in soil science is to determine the soil organic carbon (SOC) content of soil samples. Under the Kyoto Protocol (1997) the recommended way to determine this is to use dry combustion, but the equipment needed for this is expensive to buy and maintain. A much cheaper alternative is to use the Loss on Ignition (LOI) technique in which pre-dried soil samples are heated in a furnace at about 550C for a few hours. This burns off any organic matter and enables the soil organic matter (SOM) content to be determined. The task then is to convert SOM to SOC. 


## Simple linear regression: Body mass vs metabolic rate
...
```{r}
power_law_data_augmented <- power_law_data |> 
  mutate(log_bm = log(m_kg),
         log_mbr = log(fmr_k_j_day))

mammals <- power_law_data_augmented |> filter(class == "Mammalia")
birds <- power_law_data_augmented |> filter(class == "Aves")
```
...
```{r}
mammals.model <- glm(log_mbr ~ log_bm, data = mammals)
birds.model <- glm(log_mbr ~ log_bm, data = birds)
```

```{r}
autoplot(mammals.model) + theme_cowplot()
```

```{r}
autoplot(birds.model) + theme_cowplot()
```
```{r}
